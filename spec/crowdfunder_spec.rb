...
describe "crowdfunder sql" do
  ...
  describe "questions" do
    ...
    SQL
      ...
      expect(@db.execute(sql)).to eq([["Animal shelter needs dog food", 210], ["Help me buy a guitar", 98], ["Help save birds of paradise", 170], ["I have bed bugs!", 740], ["I want to teach English in China", 200], ["Iguana needs tail operation", 1035.5], ["My book on SQL", 20], ["The next Harry Potter", 120], ["The next Inna-Gadda-Davida", 342], ["Voldement needs a body", 489]])
    end
...
sql = <<-SQL
    ...
    SQL
      ...
      expect(@db.execute(sql)).to eq([["Albus", 113, 470], ["Alex", 33, 20], ["Amanda", 24, 40], ["Bear", 6, 50], ["Ena", 24, 100], ["Finnebar", 17, 70], ["Franz", 100, 90], ["Hermione", 30, 50], ["Iguana", 4, 10], ["Katie", 24, 170], ["Marisa", 24, 24], ["Pacha", 5, 60], ["Rosey", 9, 50], ["Sirius", 36, 19], ["Sophie", 24, 60], ["Squid", 5, 270], ["Swizzle", 4, 12], ["Victoria", 23, 1700], ["Voldemort", 90, 34], ["Whale", 6, 125.5]])
    end

    it 'selects the titles of all projects that have met their funding goal' do

      sql = <<-SQL
    SELECT projects.title, (projects.funding_goal - SUM(pledges.amount)) AS "this"
    FROM projects
    INNER JOIN pledges
    ON projects.id = pledges.project_id
    ...
    ORDER BY "this" DESC
    SQL
      expect(@db.execute(sql)).to eq([["My book on SQL", 0], ["The next Inna-Gadda-Davida", -142]])
    end

    it 'selects user names and amounts of all pledges then orders them by the amount' do 

      sql = <<-SQL
    SELECT users.name, SUM(pledges.amount) AS "here-here"
    FROM users
    INNER JOIN pledges
    ON users.id = pledges.user_id
    GROUP BY users.name
    ORDER BY "here-here" ASC
    SQL
      expect(@db.execute(sql)).to eq([["Iguana", 10], ["Swizzle", 12], ["Sirius", 19], ["Alex", 20], ["Marisa", 24], ["Voldemort", 34], ["Amanda", 40], ["Bear", 50], ["Hermione", 50], ["Rosey", 50], ["Pacha", 60], ["Sophie", 60], ["Finnebar", 70], ["Franz", 90], ["Ena", 100], ["Whale", 125.5], ["Katie", 170], ["Squid", 270], ["Albus", 470], ["Victoria", 1700]])   
    end
    ...
    SQL
      expect(@db.execute(sql)).to eq([["music", 40], ["music", 24], ["music", 34], ["music", 12], ["music", 40], ["music", 40], ["music", 20], ["music", 230]])
    end
    ...
    SQL
      expect(@db.execute(sql)).to eq([["books", 140]])
    end
  end
end
